...
export default function GRNListScreen({ navigation }) {
  ...
  const markRead = async (id) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await axios.put(`/grn/${id}/mark-read`, {}, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
        timeout: 20000
      });
      if (res.data.success) {
  ...
  const openImageViewer = (images) => {
    if (images && images.length > 0) {
      setImageViewerImages(images.map(img => `${SERVER_URL}/${img}`));
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      {/* Header with GRN Number and Status */}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={true} style={styles.imagesScroll}>
            {item.images.map((img, idx) => (
              <View key={idx} style={styles.imageWrapper}>
                <Image source={{ uri: `${SERVER_URL}/${img}` }} style={styles.grnImage} />
      ...
      {/* Edit/Delete only for the original submitter and only when unread */}
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f5f5f5' }}>
      ...
      {/* (Supplier/Product moved below Date Range as requested) */}
      ...
      <View style={styles.filterRow}>
        {['All', 'Good', 'Damaged', 'Rejected'].map(cond => (
          ...
          >
            <Text style={[styles.filterBtnText, conditionFilter === cond && styles.filterBtnTextActive]}>
              {cond}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {/* Date Range Modal (two pickers side-by-side) */}
      ...
      {/* Supplier & Product selectors (appear below Date Range) */}
      ...
      {/* Supplier Modal (centered from top) */}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setSelectedSupplier(selectedSupplier?._id === item._id ? null : item);
      ...
      {/* Product Modal (centered from top) */}
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setSelectedProduct(selectedProduct?._id === item._id ? null : item);
