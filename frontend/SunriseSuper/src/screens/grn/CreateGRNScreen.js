  ...
  const fetchData = async () => {
    try {
      ...
      const [suppRes, prodRes] = await Promise.all([
        axios.get('/suppliers', { headers, timeout: 20000 }),
        axios.get('/products', { headers, timeout: 20000 })
      ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = isEditMode
        ? await axios.put(`/grn/${editGRN._id}`, formData, { headers, timeout: 20000 })
        : await axios.post('/grn', formData, { headers, timeout: 20000 });
